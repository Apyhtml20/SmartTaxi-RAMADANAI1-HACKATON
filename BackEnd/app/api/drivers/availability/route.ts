import { NextRequest, NextResponse } from 'next/server';
import { z } from 'zod';
import { getAuth } from '@/lib/auth';
import { setAvailability } from '@/lib/services/driver.service';
import { withCors, corsPreflight } from '@/lib/cors';

const schema = z.object({ isAvailable: z.boolean() });

export function OPTIONS() { return corsPreflight(); }

export async function POST(req: NextRequest) {
  const auth = getAuth(req);
  if (!auth) return withCors(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }));
  if (auth.role !== 'DRIVER') return withCors(NextResponse.json({ error: 'Forbidden' }, { status: 403 }));

  try {
    const body = schema.parse(await req.json());
    const driver = await setAvailability(auth.userId, body.isAvailable);
    return withCors(NextResponse.json({ driver }));
  } catch {
    return withCors(NextResponse.json({ error: 'Bad request' }, { status: 400 }));
  }
}
